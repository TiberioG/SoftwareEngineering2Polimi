%%%%Basic service use case definition
  ...
  \begin{enumerate}
    ...
    \item{The system reminds the user: the violation and the licence plate of the veichle which is in violation must be visible }
    ...
    \item{The system shows "brush tool mode" and the user can cover the other licence plates whith his finger. User can skip this part if threre are no other plates}
    \item{When done, user press continue button. }
    \item{The system sends the picture to the ALPR service which returns the string containing the platedecoded. User just waits}
  ...
  \begin{enumerate}
    ...
    \item{If the ALPR service returns more than one plate, the user is informed that must delete the no trequired plates and the system goes again to the "brush tool mode"}
...
\item{Advanced function}\\
    ...
    \begin{enumerate}
      ...
      \item {AuthorityUser has chosen to see the Statistics - trend option}
			\item {The system shows the count of the approved tichets of last month}
    \end{enumerate}
    \textbf{Exit conditions}:
    \begin{enumerate}
			\item{the AuthorityUser wants to go back to other sections}
